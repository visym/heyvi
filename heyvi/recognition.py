...
class PIP_370k(PIP_250k, pl.LightningModule, ActivityRecognition):
    ...
    def __init__(self, pretrained=True, deterministic=False, modelfile=None, mlbl=False, mlfl=False):
        ...
        # Generated using heyvi.dataset.Dataset(...).multilabel_inverse_frequency_weight()
        ...
        # Generated using heyvi.dataset.Dataset(...).class_to_index()
    ...
    def _totensor(v, training, validation, input_size, num_frames, mean, std, noflip=None, show=False, doflip=False, stride_jitter=3, asjson=False):
        ...
        try:
            ...
            if training or validation:
                print('[heyvi.recognition.PIP_370k._totensor][SKIPPING]: video="%s", exception="%s"' % (str(vc), str(e)))
                ...
                print('[heyvi.recognition.PIP_370k._totensor][ERROR]: discarding tensor for video "%s" with exception "%s"' % (str(vc), str(e)))
                #t = torch.zeros(4, num_frames, input_size, input_size)  # skip me (should never get here)
                raise
...
class ActivityTracker(PIP_370k): 
    ...
    def __call__(self, vi, topk=1, activityiou=0.1, mirror=False, minprob=0.04, trackconf=0.2, maxdets=105, lr_threshold=None, lr_merge_threshold=None, avgdets=70, throttle=True, buffered=True):
        ...
        try:
            ...
            nounconf = {k:t.confidence(samples=8) for (k,t) in vo.tracks().items()}   # 
